...
/********************************* Invoice ***********************************/
  ...
  public function invoice(){
    ...
    $data['invoice_list'] = $this->Master_Model->get_list_by_id3($smm_company_id,'reseller_id','0','','','','','invoice_id','DESC','smm_invoice');
    $data['page'] = 'Invoice';
    $this->load->view('Admin/Include/head', $data);
    $this->load->view('Admin/Include/navbar', $data);
  ...
  public function admin_invoice_print($invoice_id = null){
    ...
    $invoice_details = $this->Master_Model->get_info_arr_fields3('*', $smm_company_id, 'invoice_id', $invoice_id, 'reseller_id', '0', '', '', 'smm_invoice');
    if(!$invoice_details){ header('location:'.base_url().'Reseller/Res_Invoice/invoice_list'); }

    if($invoice_details[0]['reseller_id'] == 0){
      $company_info = $this->Master_Model->get_info_arr_fields3('*', $smm_company_id, '', '', '', '', '', '', 'company');
      ...
      $seller_info = $this->Master_Model->get_info_arr_fields3('*', $smm_company_id, 'reseller_id', $invoice_details[0]['reseller_id'], '', '', '', '', 'smm_reseller');
      if(!$seller_info){ header('location:'.base_url().'Finance/invoice'); }
    ...
    $data['page'] = 'Invoice';
    $this->load->view('Admin/Finance/invoice_print', $data);
  }

  // Redeem Request List...
  public function redeem_request_list(){
    $smm_user_id = $this->session->userdata('smm_user_id');
    $smm_company_id = $this->session->userdata('smm_company_id');
    $smm_role_id = $this->session->userdata('smm_role_id');
    if($smm_user_id == '' || $smm_company_id == ''){ header('location:'.base_url().'User'); }
    ...
    $data['redeem_request_list'] = $this->Master_Model->get_list_by_id3($smm_company_id,'','','','','','','redeem_request_id','DESC','smm_redeem_request');
    $data['page'] = 'Redeem Request List';
    $this->load->view('Admin/Include/head', $data);
    $this->load->view('Admin/Include/navbar', $data);
    $this->load->view('Admin/Finance/redeem_request_list', $data);
    $this->load->view('Admin/Include/footer', $data);
  }

  //  Cancel Order List...
  public function redeem_request_approve(){
    $smm_user_id = $this->session->userdata('smm_user_id');
    $smm_company_id = $this->session->userdata('smm_company_id');
    $smm_role_id = $this->session->userdata('smm_role_id');
    if($smm_user_id == '' || $smm_company_id == ''){ header('location:'.base_url().'User'); }

    $redeem_request_id = $_POST['redeem_request_id'];
    $update_data['redeem_request_status'] = '1';
    $update_data['redeem_request_approve'] = $_POST['redeem_request_approve'];
    $this->Master_Model->update_info('redeem_request_id', $redeem_request_id, 'smm_redeem_request', $update_data);

    header('location:'.base_url().'Finance/redeem_request_list');
  }
